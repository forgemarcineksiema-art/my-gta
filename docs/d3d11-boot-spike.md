...
- **Separate from `D3D11Renderer` smoke**: production-facing `D3D11Renderer` lives under `src/render_d3d11/` and has its own init/clear/present smoke path, but still does not draw `RenderFrame` primitives.
