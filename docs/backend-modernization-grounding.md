...
Backend prep status update: Stage 5 complete (5a–5d). Stage 6a DONE — `CpuMeshLoader` (`src/render/CpuMeshLoader.h/.cpp`) with minimal OBJ parsing producing `CpuMeshData`. Supports positions + triangulated faces, slash tokens, ignores harmless unsupported lines (o/g/s/usemtl/mtllib/vt/vn), rejects invalid indices/missing vertices. 14 data-only tests. No D3D11, no GameApp integration, no textures/materials. `--renderer d3d11` still inactive.
