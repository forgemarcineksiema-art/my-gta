...
Backend prep status update: Stage 5d DONE — `selectShadowMeshSeedAssetIds` deterministic helper replaces ad-hoc opaque-only seeding. Iterates WorldRenderList by bucket order (Opaque→Vehicle→Decal→Glass→Translucent), deduplicates, skips missing definitions. Seed IDs logged when `--d3d11-shadow-diagnostics` is active. No real asset loading. No GameApp main renderer changes. `--renderer d3d11` still inactive.
