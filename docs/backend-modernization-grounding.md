...
Backend prep status update: `RenderFrameDump v2` is implemented and exposed through GameApp CLI (`--renderframe-shadow-dump-version v2`) and capture/replay script (`-DumpVersion v2`). v1 remains default. v2 is metadata-only — no geometry, textures, material definitions, or asset loading. Reader accepts both v1 and v2 headers. `--renderer d3d11` remains inactive. Runtime renderer is still raylib.
