...
Backend prep status update: Stage 5c DONE — D3D11 shadow sidecar uploads procedural meshes for emitted MeshHandle ids. Stage 5d DONE — `selectShadowMeshSeedAssetIds` deterministic helper replaces ad-hoc opaque-only seeding, iterating buckets in production order with deduplication. Seed IDs logged when `--d3d11-shadow-diagnostics` is active. No real asset loading. No GameApp main renderer changes. `--renderer d3d11` still inactive.
