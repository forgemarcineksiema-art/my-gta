...
Backend prep status update: Stage 5 complete (5a: extraction helper, 5b: GameApp shadow wiring, 5c: sidecar procedural mesh upload, 5d: deterministic seed selection). Stage 6 planned — CpuMeshLoader for real mesh asset data from `WorldAssetDefinition.modelPath` (minimal OBJ subset first). No texture loading, no material system, no GameApp main renderer. `--renderer d3d11` still inactive.
