...
Flaga `--orbit-camera` włącza prostą kamerę inspekcyjną z kontrolkami klawiatury (strzałki/A/D = yaw, W/S = zoom, Q/E = wysokość, R = reset, F5 = przeładowanie pliku dump). `--auto-orbit` dodaje powolną automatyczną rotację. Obsługuje hot-reload pliku `RenderFrameDump v1` przez F5 bez restartowania shella.
