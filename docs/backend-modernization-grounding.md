...
Backend prep status update: Stage 5a started — `addWorldRenderListMeshCommands` data-only helper exists in `RenderExtraction.h/.cpp`. Emits Mesh commands from WorldRenderList when MeshRegistry has the assetId, falls back to Box when missing. Uses `MaterialRegistry::defaultOpaque()` for Opaque/Vehicle/Decal buckets and `defaultAlpha()` for Glass/Translucent. Stats struct extended with `emittedMeshes`/`meshFallbacks`/`missingDefinitions`. Not wired to GameApp yet — helper is test/extraction infrastructure only. No CLI/runtime behavior changed. `--renderer d3d11` still inactive.
