...
Backend prep status update: `RenderFrameDump v2` is implemented (`src/render/RenderFrameDump.h/.cpp`). v2 supports Mesh command metadata round-trip via `RenderFrameDumpWriteOptions({RenderFrameDumpVersion::V2})`. v1 remains the default write format and is unchanged. v2 is handle-metadata only — no geometry, textures, material definitions, or asset loading. Reader accepts both v1 and v2 headers. `--renderer d3d11` remains inactive. Runtime renderer is still raylib.
