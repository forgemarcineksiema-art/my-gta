...
`RenderFrameDump v1` celowo nie serializuje nieobsługiwanych typów prymitywów (Sphere, Mesh, CylinderX, QuadPanel) — tylko prymitywy Box i debug lines przechodzą przez round-trip. Testy `dumpSkipsNonBoxPrimitivesOnWrite` i `dumpRoundTripWithBoxAndNonBoxValidatesOnlyBoxSurvives` potwierdzają to oczekiwane ograniczenie v1.
