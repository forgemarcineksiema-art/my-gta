...
Backend prep status update: Stage 5 planned — GameApp shadow extraction may emit Mesh commands via `--renderframe-shadow-meshes` (dev-only). `MeshRegistry` maps `WorldObject.assetId` → `MeshHandle`. Fallback Box extraction remains for unresolved handles. No real asset loading. `--renderer d3d11` remains inactive. Runtime renderer is still raylib.
