...
Backend prep status update: `docs/d3d11-mesh-material-pipeline-plan.md` now reflects implemented reality: Stages 1-2 marked DONE, Stage 3 in progress (3a data+adapter DONE, 3b shell test mesh NEXT). `D3D11Renderer::initialize()` built-in cube upload now uses `CpuMeshData` → `D3D11MeshUploadAdapter` → `D3D11MeshCache` pipeline.
