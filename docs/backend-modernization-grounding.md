...
Diagnostyka zawiera teraz współczynniki pokrycia (procentowe): `supportedBoxes`, `supportedMeshes`, `supportedPrimitives` (liczby wspieranych prymitywów w shadow frame), `boxCoveragePct` (drawnBoxes / supportedBoxes * 100), `meshCoveragePct` (drawnMeshes / supportedMeshes * 100), `primitiveCoveragePct` ((drawnBoxes + drawnMeshes) / supportedPrimitives * 100), `lineCoveragePct` (drawnDebugLines / debugLines * 100). Mesh id=1 pozostaje ścieżką smoke/test-only — bez prawdziwego ładowania assetów.
...
`RenderFrameDump v1` celowo nie serializuje nieobsługiwanych typów prymitywów (Sphere, Mesh, CylinderX, QuadPanel) — tylko prymitywy Box i debug lines przechodzą przez round-trip. Nawet wbudowany `MeshHandle id=1` (wspierany przez D3D11Renderer przez smokową ścieżkę unit cube) nie jest serializowany w dump v1. Testy `dumpSkipsNonBoxPrimitivesOnWrite` i `dumpRoundTripWithBoxAndNonBoxValidatesOnlyBoxSurvives` weryfikują to zachowanie.
...
Flaga `--orbit-camera` włącza prostą kamerę inspekcyjną z kontrolkami klawiatury (strzałki/A/D = yaw, W/S = zoom, Q/E = wysokość, R = reset, F5 = przeładowanie pliku dump). `--auto-orbit` dodaje powolną automatyczną rotację. `--wire-boxes` włącza opcjonalny wireframe overlay na prymitywach Box i Mesh. `--add-test-mesh` dodaje wbudowany testowy prymityw Mesh (id=1, unit cube). Obsługuje hot-reload pliku `RenderFrameDump v1` przez F5 bez restartowania shella.
D3D11Renderer wspiera tymczasowo wbudowany MeshHandle id=1 (unit cube) używający tej samej geometrii co Box. Brak prawdziwego ładowania assetów/meshów, materiałów i tekstur.
