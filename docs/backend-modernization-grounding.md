...
Diagnostyka zawiera teraz współczynniki pokrycia (procentowe): `supportedBoxes`, `supportedMeshes`, `supportedPrimitives` (liczby wspieranych prymitywów w shadow frame), `boxCoveragePct` (drawnBoxes / supportedBoxes * 100), `meshCoveragePct` (drawnMeshes / supportedMeshes * 100), `primitiveCoveragePct` ((drawnBoxes + drawnMeshes) / supportedPrimitives * 100), `lineCoveragePct` (drawnDebugLines / debugLines * 100). `BuiltInUnitCubeMeshId` pozostaje ścieżką smoke/test-only — bez prawdziwego ładowania assetów.
...
100% boxCoveragePct/lineCoveragePct oznacza 100% wspieranego podzbioru Box/debug-line — nie pełną kompletność renderera. `Mesh`, `Sphere`, `CylinderX`, `QuadPanel` są nadal nieobsługiwane przez D3D11Renderer.
`RenderFrameDump v1` celowo nie serializuje nieobsługiwanych typów prymitywów (Sphere, Mesh, CylinderX, QuadPanel) — tylko prymitywy Box i debug lines przechodzą przez round-trip. Nawet wbudowany `BuiltInUnitCubeMeshId` (tymczasowy unit cube wspierany przez D3D11Renderer przez smokową ścieżkę) nie jest serializowany w dump v1. Testy `dumpSkipsNonBoxPrimitivesOnWrite` i `dumpRoundTripWithBoxAndNonBoxValidatesOnlyBoxSurvives` weryfikują to zachowanie.
...
Flaga `--diagnostics` wypisuje statystyki załadowanej ramki (walidacja, podsumowanie bucketów) oraz pokrycie rysowania D3D11 (drawnBoxes, drawnMeshes, skipKinds, skipBuckets, skipPrims, drawnLines, skipLines).
Flaga `--orbit-camera` włącza prostą kamerę inspekcyjną z kontrolkami klawiatury (strzałki/A/D = yaw, W/S = zoom, Q/E = wysokość, R = reset, F5 = przeładowanie pliku dump). `--auto-orbit` dodaje powolną automatyczną rotację. `--wire-boxes` włącza opcjonalny wireframe overlay na prymitywach Box i Mesh. `--add-test-mesh` dodaje wbudowany testowy prymityw Mesh (`BuiltInUnitCubeMeshId`, unit cube). Obsługuje hot-reload pliku `RenderFrameDump v1` przez F5 bez restartowania shella.
D3D11Renderer wspiera tymczasowo wbudowany `BuiltInUnitCubeMeshId` (unit cube) używający tej samej geometrii co Box. Brak prawdziwego ładowania assetów/meshów, materiałów i tekstur.
...
Backend prep status update: Stage 3b complete — `D3D11GameShell` can upload a procedural `CpuMeshData` triangle through `D3D11Renderer::uploadTestMesh(MeshHandle{2})` and render it alongside `BuiltInUnitCubeMeshId`. `--add-test-mesh` diagnostics show `drawnMeshes=2`. `--renderer d3d11` is still NOT active. Runtime renderer remains raylib.
...
Backend prep status update: Stage 5b wired — `--renderframe-shadow-meshes` CLI flag active in GameApp shadow path. Enables `addWorldRenderListMeshCommands` extraction behind the flag. Seeds MeshRegistry with first 3 assetIds from WorldRenderList opaque objects. Falls back to Box for unregistered assets. Diagnostics log `emittedMeshes`/`meshFallbacks`/`missingDefinitions`. Mesh commands survive capture only with `--renderframe-shadow-dump-version v2`; v1 remains default and still skips Mesh. No D3D11 mesh upload, no real asset loading, no raylib runtime changes. `--renderer d3d11` still inactive.
