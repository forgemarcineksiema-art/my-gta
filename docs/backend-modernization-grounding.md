...
- **Partial/Aspirational (Planned runtime values; NOT active)**:
  - renderer: `d3d11` (`D3D11Renderer` private init/depth path plus tiny standalone-smoke Box/debug-line drawing subset; runtime selection/full primitive drawing not implemented)
  - fizyka: `physx`
...
| d3d11 | custom WorldCollision | Standalone boot spike draws; production `D3D11Renderer` can init/clear/depth/present and draw a tiny depth-tested Box/debug-line subset in smoke while recording `RenderFrame` stats/validation; runtime renderer NOT implemented |
