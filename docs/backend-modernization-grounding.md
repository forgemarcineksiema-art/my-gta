...
Backend prep status update: Stage 5c implemented — D3D11 shadow sidecar uploads procedural `makeCpuMeshUnitCube` meshes for MeshHandle ids seeded by `--renderframe-shadow-meshes`. Sidecar `uploadTestMesh` bridge forwards to `D3D11Renderer::uploadTestMesh`. 3 procedural unit cubes uploaded once after seeding. Diagnostics show `drawnMeshes=5` when both `--renderframe-shadow-meshes` and `--d3d11-shadow-window` are active. No real asset loading. No GameApp main renderer changes. `--renderer d3d11` still inactive.
