...
Stage 6 status: DONE — Stage 6c completes the initial asset-loading pipeline.
  ...
  - Option A (DONE): Broader WorldAssetRegistry integration. `selectShadowMeshSeedAssetIdsFromDefinitions` with modelPath preference, `renderInGameplay` filtering. Current seed limit 16, one-time shadow snapshot. sample smoke: `drawnMeshes=36` with sidecar.
